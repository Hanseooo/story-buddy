...
- **Pipeline Data**: Using the donated Grade 5–6 stories (ten primary + five backup), the system generates canonical character references and corresponding scene images.
...
- **Constructed Negatives**: Hard negatives are generated by pairing a character's reference with a scene generated for a *different* character requiring mandatory species and style match, then maximizing similarity over colour/body/silhouette/clothing/face. These are effectively free to produce and highly informative for the model.
...
The evaluation architecture separates the *reported research finding* (**Objective 4** — the fine-tuned judge's classification performance against human labels) from the *engineering* deployment decision ("Should this judge replace the one in the product?"). Per **ADR-008 (revised 2026-07-25)**, Objective 4 reports the fine-tuned judge's **precision, recall, and F1 (F1 primary)** against human reference labels on the character-disjoint held-out set; an **optional** secondary comparison against the zero-shot base and the prompted baseline is permitted. The deployment gate below is the separate build/deployment machinery. 
...
### 4.2 Metrics
- **Reported Metric (Objective 4):** F1, precision, and recall on the `different_character` class on the held-out, human-labeled test set (F1 primary), with 95% bootstrap confidence intervals resampled by character, not by pair. This is the paper's Objective-4 finding.
...
To prevent moving goalposts, the ship/no-ship decision is categorized into a pre-registered ladder. This is a **deployment/build gate**, separate from the Objective-4 finding — the ship/no-ship *interpretations* below are build decisions, not the reported classification metrics (§4.2). The δ = 3 non-inferiority gate itself is ADR-018 and is unaffected:
...
Whichever rung lands, the paper reports the same Objective-4 finding: the fine-tuned judge's classification performance (precision/recall/F1) against human labels, plus which judge the deployment gate selected.
