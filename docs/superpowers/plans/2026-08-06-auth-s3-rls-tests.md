# Auth Authorization Surface — Tier-A Isolation Tests Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Write 33 Tier-A isolation tests that verify every RLS policy in `0008_authorization_surface.sql` — one test per spec §6 case, structured so that dropping any single policy causes exactly the tests that depend on it to fail.

**Architecture:** One pytest file using a module-scoped psycopg connection and a committed fixture that creates two classrooms, six users (2 teachers, 3 students, 1 researcher), three jobs, and three `storage.objects` rows. Each test runs in its own transaction (rollback autouse fixture), switches role + JWT claims via `SET LOCAL`, and asserts row visibility. Realtime tests 26–27 are not automated here — their SELECT policy coverage is provided by tests 1–4.

**Tech Stack:** pytest, psycopg (already in backend deps from `test_auth_schema.py`), local Supabase instance with migrations 0007 + 0008 applied.

**Prerequisite:** Complete Plan 1 (`2026-08-06-auth-s3-migration.md`) first — `0008` must be applied before running these tests against a live DB.

## Global Constraints

- Tests skip automatically in CI (`SUPABASE_DB_URL` resolves to a non-connectable dummy — same pattern as `test_auth_schema.py`)
- All 33 test names are prefixed `test_NN_` matching spec §6 numbering
- Zero new Python packages — psycopg is already available
- `jobs` INSERT uses `'complete'` status and `input_text = 'test story'` (satisfies all NOT NULL / CHECK constraints after `0008`)
- Fixture is module-scoped and committed; teardown deletes in reverse FK order
- Each test uses the `rollback` autouse fixture — modifications during UPDATE tests do not bleed into subsequent tests
- Realtime tests 26–27: documented as skipped with rationale (policy coverage by tests 1, 2, 4)
...
| File | Change |
|---|---|
| `backend/tests/test_rls_isolation.py` | **Create** — all 33 Tier-A isolation tests |

---

### Task 1: Test file skeleton — fixtures and helpers

**Files:**
- Create: `backend/tests/test_rls_isolation.py`

**Interfaces:**
- Consumes: `SUPABASE_DB_URL` env var (set by `conftest.py` to dummy; override with real URL to run)
- Produces: `RLSFixture` dataclass, `conn` fixture, `rollback` fixture, `fx` fixture, all `_as_*` / `_select_*` / `_update_*` helpers — used by Task 2 tests
...
Expected: all pass. Do not proceed if anything is red.
...
- [ ] **Step 3: Run — confirm 0 collected (file is importable, no test functions yet)**

```bash
cd backend && uv run pytest tests/test_rls_isolation.py -v
```

Expected: `collected 0 items` (or all skip if `SUPABASE_DB_URL` is the dummy). No import errors.

- [ ] **Step 4: Commit the skeleton**

```bash
git add backend/tests/test_rls_isolation.py
git commit -m "test(auth-s3): RLS isolation test skeleton — fixtures and helpers"
```

---

### Task 2: All 33 isolation test functions

**Files:**
- Modify: `backend/tests/test_rls_isolation.py` (append all test functions)

**Interfaces:**
- Consumes: `conn`, `rollback`, `fx`, `_skip`, all `_select_*` / `_update_*` / `_as_*` helpers defined in Task 1
- Produces: 31 runnable test functions (tests 1–25, 28–33) + 2 documented-skip stubs (26–27) that all pass when `0008` is applied

- [ ] **Step 1: Append all test functions**

Append to `backend/tests/test_rls_isolation.py`:
...
- [ ] **Step 2: Run in CI mode — confirm all skip cleanly**

```bash
cd backend && uv run pytest tests/test_rls_isolation.py -v
```

Expected: 31 tests collected, all marked `SKIPPED` (SUPABASE_DB_URL dummy is unreachable).
No `ERROR` or `FAILED`. If any `ERROR`, there is a syntax/import error — fix it.

- [ ] **Step 3: Run against local Supabase — confirm all pass**

First confirm migrations are applied:
```bash
supabase status
```

Then run with the real local DB URL (default Supabase local port is 54322):
```bash
cd backend
SUPABASE_DB_URL=postgresql://postgres:postgres@localhost:54322/postgres \
uv run pytest tests/test_rls_isolation.py -v
...
Expected: 31 tests pass, 0 fail, 0 error, 0 skip.

**If tests fail:** The most common causes:
- `SET LOCAL ROLE authenticated` error → your local DB role `authenticated` does not exist; run `supabase db reset` to replay migrations which create it.
- `permission denied for table storage.objects` → the psycopg connection user needs superuser or `storage` schema rights; connect as `postgres` (the default local Supabase superuser).
- A test that should be `allowed` returns 0 rows → the corresponding policy is wrong in `0008`; check the policy name and `using` clause against spec §4.
- A test that should be `denied` returns 1 row → the anon policy was not dropped, or a permissive policy is too broad.

- [ ] **Step 4: Run full backend suite — confirm no regressions**

```bash
cd backend && uv run pytest -v
```

Expected: all existing tests green, new RLS tests either pass (with real DB) or skip (without).

- [ ] **Step 5: Commit**

```bash
git add backend/tests/test_rls_isolation.py
git commit -m "test(auth-s3): 33 Tier-A classroom isolation tests (spec §6)"
```
