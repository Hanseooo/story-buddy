"""Hypothesis 2: D1 fails because it sits as a standalone bullet about characters_present,
away from the bullet that actually defines key_action. Move the constraint inside that
definition and re-measure. Text-only.
"""
...
assert full.count(OLD_KA) == 1
inline = stripped.replace(OLD_KA, NEW_KA)
assert inline != stripped

for arm, prompt in (("inline-in-key_action", inline),):
