...
| **Unknown object owner** | Object kept unowned, warning logged — covers an owner capped out of the roster and one that was never a character (a group, a relative who is not extracted). |
...
- **Object ownership:** maps `owner_name` to `owner_char_id`, keeps objects unowned if the owner is not a character or was capped (`test_analyze_maps_owner_name_to_the_capped_character_id`, `test_analyze_keeps_an_object_whose_owner_is_not_a_character`, `test_analyze_keeps_an_object_whose_owner_was_capped_out_of_the_roster`)
