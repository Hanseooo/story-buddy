...
- **Current phase: Phase 2 (Safety / Classroom) — in progress.** Phase 0 ✅ done. Phase 1 ✅ complete.
  ...
  **`0020`** — ⚠️ `0019` is the highest on disk (`0019_jobs_title.sql`, applied 2026-09-10) and **`0009` is used twice**
...
- story-failure-recovery-ux (2026-09-09): child failure copy moved out of a per-reason branch into `frontend/lib/failureCopy.ts`, keyed on the eight ADR-038 reasons; null/`machine`/unknown fall back to `system_error`. `service_limit` and `book_limit` carry no new-job action at all (`failure-diagnostics.md` CC-3). Whole-book consequence text sits before the retry button, clipboard failure is reported honestly, and a failed *read* offers a re-read rather than a paid rebuild.
- story-titles (2026-09-10): `jobs.title` (nullable, legacy rows keep null) + required `CreateStorybookRequest.title`, `MAX_TITLE_CHARS = 80` in `app/config.py`, checked and redacted synchronously by `providers.check_text` before persistence, `title_ack` for the privacy-replacement round trip. ADR-059. Migration `0019_jobs_title.sql`, applied 2026-09-10. `frontend/lib/displayTitle.ts` is the one display rule: stored title → first-line/60-char excerpt → `Untitled`. The excerpt fallback is **owner-only** — the classroom gallery selects `title` alone and the reader passes `null` for a classmate's book, because `jobs.input_text` is the raw pre-redaction story and RLS hands a whole approved row to any classmate.
- character-review-ux (2026-09-10): the reveal chip no longer posts on tap. Selecting a trait is local state and spends nothing; an explicit **Redraw [name]** dispatches the unchanged `{action: "try_again", char_id, attribute}` payload once. Allowance always comes from server `taps_left` — never decremented locally. Backend reveal/confirm contracts (`pipeline/reveal.py`, `pipeline/char_bible.py`, `POST /jobs/{id}/confirm`) are untouched by design.
