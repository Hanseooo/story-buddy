...
def test_over_cap_with_blank_line_in_range_cuts_at_paragraph():
    # Two paragraphs; the first ends well past the floor (half of MAX_STORY_WORDS = 150).
    first_paragraph = _words(200)
    second_paragraph = _words(200)
...
def test_over_cap_no_blank_line_sentence_punctuation_present_cuts_at_sentence():
    ...
    # the cap keeps at least the floor (150 words).
    sentence_a = _words(200) + "."
    sentence_b = _words(200) + "."
...
def test_floor_guard_fires_when_blank_line_is_too_early():
    ...
    # floor (150 words), so it must fall through to the sentence/hard cut instead.
    early_paragraph = _words(10)
    rest = _words(500)  # no further blank line, no punctuation
...
def test_over_cap_no_punctuation_no_blank_lines_hard_cuts_at_max():
    text = _words(500)  # one run-on, no ".", no "\n\n"
    result, truncated = clamp_story(text)
    assert truncated is True
    assert word_count(result) == MAX_STORY_WORDS
